...
```sh
Rscript 2_98_WIP/tests/run_all.R          # all fast gates, one line each, exit 0 = all passed
Rscript 2_98_WIP/tests/test_preflight.R   # or any single gate
```

> **On the `2_98_WIP/` prefix in every command below.** These paths are written for the DEVELOPMENT
...
```sh
Rscript 2_98_WIP/tests/run_all.R                       # from the repo root
(cd 2_98_WIP && Rscript tests/run_all.R)                # from the version folder
(cd /tmp && Rscript "$ACTA/2_98_WIP/tests/run_all.R")   # from anywhere at all
```
...
(`Rscript 2_98_WIP/tests/test_preflight.R 2_89`); a path that does not exist is a **hard error**,
...
```r
h <- new.env(); sys.source("2_98_WIP/ACTA_Functions.R", envir = h)
r <- h$actaOQRun("2_98_WIP/Diagnostics/OQ_Test1")   # writes Outputs/log.txt with the verdict table
```
...
```sh
Rscript 2_98_WIP/tests/regression_capture.R 2_98_WIP baseline.rds   # before the change
#   ... make the change ...
Rscript 2_98_WIP/tests/regression_capture.R 2_98_WIP after.rds      # after
Rscript 2_98_WIP/tests/regression_compare.R baseline.rds after.rds  # exit 0 = identical
```
